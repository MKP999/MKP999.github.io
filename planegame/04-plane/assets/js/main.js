...
Element.prototype.createBullet = function() {
    game.players.forEach(function(player, index, players) {
        ...
        if (player.buff === true) {
            ...
            var lefts = new Element(typeOurBullet, player.x - 10, player.y);
            var rights = new Element(typeOurBullet, player.x + 10, player.y);
            left.draw();
            right.draw();
            lefts.draw();
            rights.draw();
            player.bullets.push(left, right, lefts, rights);
        } else {
...
Game.prototype.createPlayer = function() {
    ...
    if (document.body.clientWidth <= 768) {
        document.querySelector(".game .score .buff").style.display = "block";
    }
};
...
// 加buff
document.querySelector(".game .score .buff").ontouchend = function(event) {
    // console.log('xxx');
